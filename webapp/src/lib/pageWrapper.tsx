...
import { lastNonAuthRouteVisitedStore } from "../components/shared/RouteNotAuth";
...
const CheckAccessFn = <T,>(value: T, message?: string): NonNullable<T> => {
    if (!value) {
...
class GetAuthorizeMeError extends Error {};
...
type SetPropsProps<TQueryResult extends QueryResult | undefined> = HelperProps<TQueryResult> & {
    ...
    getAuthorizeMe: (message?: string) => NonNullable<AppContext['me']>
}
type PageWrapperProps<TProps extends Props, TQueryResult extends QueryResult | undefined> = {
    ...
    // this is when we didn't have the error classes
...
const PageWrapper = <TProps extends Props = {}, TQueryResult extends QueryResult | undefined = undefined>({
    authorizedOnly,
    authorizedOnlyTitle = "Authorized Only",
    authorizedOnlyMessage = "This page is only for authorized users",
    redirectAuthorized,
    checkAccess,
    checkAccessTitle = "Access Denied",
    checkAccessMessage = "You have no access",
    checkExists,
    checkExistsTitle,
    checkExistsMessage,
    useQuery,
    ...
    useEffect(() => {
        if (redirectNeeded) {
            navigate(nonAuthRouteLastVisited, { replace: true })
        }
    }, [redirectNeeded, navigate, nonAuthRouteLastVisited])
    ...
    const getAuthorizeMe = (message?: string) => {
        if (!ctx.me) {
            throw new GetAuthorizeMeError(message)
        }
        return ctx.me;
    }
    ...
    try {
        ...
        if (error instanceof GetAuthorizeMeError) {
            return <ErrorPage title={authorizedOnlyTitle} message={authorizedOnlyMessage} />
        }
        throw error;
    }
...
// Call withWrapper on the page using PageWrapperProps as main type. Excludes 'Page' attribute by using Omit.
// Pass 'Page' in separately and return PageWrapper with Page inside
